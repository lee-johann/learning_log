...
- how to address this: assign "credit" to each step to filter bad steps
	...
	- (Q function the NLP folks call PRM, process reward model, the other way is to use rollouts)
...
- using this advantage for training
	- option 1: filter steps for good advantages (even if overall traj incorrect, we keep good steps) <– like advantage weighted regression but filter instead of e^a
		- results: helped w/ scaling

- how to use advantage for offline RL (DPO)?
	- option 2: retain partial rollouts from 